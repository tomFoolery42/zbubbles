...
/// resources
pub fn init(allocator: Allocator, queue: *ui.Queue) !App {
    return .{
        .allocator = allocator,
...
pub fn run(self: *App, widget: vxfw.Widget, opts: Options) anyerror!void {
    ...
    while (true) {
        ...
        while (self.queue.next()) |event| {
            loop.postEvent(.{.app = .{.name = "Ui Event", .data = event}});
        }

        try self.checkTimers(&loop);
...
fn checkTimers(self: *App, loop: *EventLoop) anyerror!void {
